...
def test_ingest_fills_runtime_defaults_without_graph(tmp_path: Path, db: ComicBookDB) -> None:
    ...
    delta = ingest(
        {
            ...
            "image_results": [{"fingerprint": "fp-existing", "status": "failed", "failure_reason": "retry later"}],
        },
        deps,
    )
...
def test_summarize_writes_artifacts_and_finalizes_run_without_graph(tmp_path: Path, db: ComicBookDB) -> None:

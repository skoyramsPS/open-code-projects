...
def make_deps(tmp_path: Path, templates: list[TemplateSummary]) -> Deps:
    ...
    return Deps(
        ...
        pricing={"image": {}},
        logger=logging.getLogger("test-load-templates"),
        pid_provider=lambda: 123,
        hostname_provider=lambda: "host-a",
    )


def test_load_templates_keeps_full_catalog_when_count_is_30_or_fewer(tmp_path: Path) -> None:
    templates = [
        make_template(
            "storybook-soft",
            name="Storybook Soft",
...
def test_load_templates_prefilters_large_catalog_before_router(tmp_path: Path) -> None:
    templates = [
        make_template(
            "heroic-forest",
            name="Heroic Forest",

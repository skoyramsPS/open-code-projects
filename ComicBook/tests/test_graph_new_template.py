...
def make_router_response() -> dict[str, Any]:
    return {
        "output": [
            {
                "type": "message",
...
def make_deps(tmp_path: Path, db: ComicBookDB, router_transport: FakeRouterTransport, image_transport: FakeImageTransport) -> Deps:
    return Deps(
        ...
        logger=logging.getLogger("test-graph-new-template"),
...
def test_run_workflow_extract_new_template_persists_exactly_one_template_row(tmp_path: Path, db: ComicBookDB) -> None:
    ...
    router_transport = FakeRouterTransport(responses=[make_router_response()])

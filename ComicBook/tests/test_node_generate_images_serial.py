...
def make_deps(tmp_path: Path, db: ComicBookDB, transport: FakeImageTransport) -> Deps:
    return Deps(
        ...
        pricing={"image": {}},
        logger=logging.getLogger("test-generate-images-serial"),
...
def test_generate_images_serial_resumes_existing_file_and_generates_remaining_prompt(
    tmp_path: Path,
    db: ComicBookDB,
...
def test_generate_images_serial_continues_after_non_retryable_failure(
    tmp_path: Path,
    db: ComicBookDB,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_generate_images_serial_stops_after_two_consecutive_rate_limit_failures(
    tmp_path: Path,
    db: ComicBookDB,
    monkeypatch: pytest.MonkeyPatch,
) -> None:

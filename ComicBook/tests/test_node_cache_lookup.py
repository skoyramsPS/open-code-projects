...
from comicbook.fingerprint import compute_prompt_fingerprint, render_prompt_text
...
def make_deps(tmp_path: Path, db: ComicBookDB) -> Deps:
    ...
    return Deps(
        ...
        pricing={"image": {}},
        logger=logging.getLogger("test-cache-lookup"),
...
def test_cache_lookup_partitions_cache_hits_and_deduplicates_generation_queue(tmp_path: Path, db: ComicBookDB) -> None:
...
def test_cache_lookup_force_regenerate_ignores_existing_successful_image(tmp_path: Path, db: ComicBookDB) -> None:
    from comicbook.nodes.cache_lookup import cache_lookup
    ...
    rendered_prompt = render_prompt_text("Traveler portrait at sunrise.", [make_template_record(storybook.id, style_text=storybook.style_text)])
...
def test_cache_lookup_does_not_treat_failed_images_as_cache_hits(tmp_path: Path, db: ComicBookDB) -> None:
    from comicbook.nodes.cache_lookup import cache_lookup

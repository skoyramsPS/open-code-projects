...
- TG4 is now in progress: node-level lifecycle logging is wired through reusable decorators around every image-workflow node, every template-upload node, and the graph-local helper nodes (`runtime_gate`, `prepare_deferred_retry`), with focused log-shape tests proving that representative end-to-end runs emit parseable JSON records carrying `workflow`, `run_id`, `event`, and `node`.
- The remaining TG4 work is the approval-sensitive template-upload rename slice (`upload_*` module/function cleanup plus wrapper rewires).

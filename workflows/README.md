...
- the TG2 test relocation sweep now centers the canonical pytest tree under
  `workflows/tests/`, with image, template-upload, and shared coverage all
...
- target-root router-node coverage is now also in place under
  `workflows/tests/image_prompt_gen/test_router_node.py`, so the remaining
  legacy `ComicBook/tests/` regression tree has been retired.
- The live runtime code still mostly lives under `ComicBook/comicbook/`; the
  canonical tests now live under `workflows/tests/`, but the runtime itself is
  still mid-migration.

"""Managed runtime dependency construction helpers shared across workflows."""
...
def load_pricing(pricing_path: Path | None = None) -> dict[str, Any]:
    """Load static pricing metadata used for best-effort cost estimates."""

    resolved = Path(pricing_path) if pricing_path is not None else _default_pricing_path()
